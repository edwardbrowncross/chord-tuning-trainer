...
- **state/types** — re-exports `ExerciseResult` from exercise types. `LevelProgress` (exerciseIndex, exercises, results), `ProgressPhase` discriminated union (`module-select | level-ready | level-active | level-complete`), `ProgressState` (phase, modules, part, moduleScores), `ProgressAction` union.
...
- **Part** — `'bass' | 'bari' | 'lead' | 'tenor'` voice part union, used across exercise and progress modules.
