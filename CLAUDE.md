...
- **components/SettingsModal** — modal for user-configurable audio settings. Exposes `vocalTractSize` as a `Button.Group` with 5 named presets (Bass 0.22 → Child 0.46), `pitchOffset` as a `Slider` (-4 to +8 semitones), and `stereo` as a `Switch`. Reads and writes via `useAudioSettings()`.
