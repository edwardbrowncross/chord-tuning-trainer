...
- **state/exerciseGenerator** — `generateExercises(level, part, audioSettings?)` converts a `LevelSpecification`, voice `Part`, and optional `AudioSettings` into an `Exercise[]`. Picks random root notes within the singer's vocal range, computes just-intonation chord notes via `getMidiChord`, resolves vowel formants, generates 3 chord voices (all except the user's part), and applies a random pitch offset to the reference tone. `audioSettings` fields (e.g. `vocalTractSize`) are forwarded as overrides to every `makeVoice` call. `PART_RANGES` defines MIDI ranges per voice part; `PART_INDEX` maps part name to 0–3 index.
